...
import project.moonki.dto.kakao.KakaoProfileDto;
...
public class KakaoService {
    ...
    public LoginResponseDto loginWithKakao(String code) {
        ...
        try {
            // 토큰 교환 & 사용자 조회
            KakaoTokenResponse token = kakaoClient.exchangeToken(code);
            log.info("[KakaoService] 토큰 발급 성공");
            KakaoUserResponseDto me = kakaoClient.fetchUser(token.getAccessToken());
    ...
    private String resolveEmail(Object me) {
        KakaoAccountDto account = getAccount(me);
    ...
    private String resolveNickname(Object me) {
        KakaoAccountDto account = getAccount(me);
        KakaoProfileDto profile = (account != null) ? account.getProfile() : null;
    ...
    private KakaoAccountDto getAccount(Object me) {
        try {
            Method method = me.getClass().getMethod("getKakao_account");
    ...
    private Long getId(Object me) {
        try {
            Method method = me.getClass().getMethod("getId");
